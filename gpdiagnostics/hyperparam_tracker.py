...
class HyperparameterTracker:
    ...
    def plot_evolution(
        ...
        fig, axes = plt.subplots(len(plot_params), 1, figsize=figsize, squeeze=False)
        axes = axes.flatten()
        ...
        for idx, name in enumerate(plot_params):
            ...
            ax.set_title(
                f"{name.replace('_', ' ').title()} Evolution", fontweight="bold"
            )
        ...
        plt.suptitle("Hyperparameter Optimization Trajectories", fontsize=16, y=0.995)
        plt.tight_layout()
        return fig
        ...
        for name, values in self.history.items():
            ...
            report[name] = {
                ...
                "relative_change": float(
                    abs(np.mean(recent) - np.mean(earlier))
                    / (abs(np.mean(earlier)) + 1e-10)
                ),
                "final_std": float(np.std(recent)),
                "converged": float(np.std(recent))
                < 0.01 * float(np.mean(np.abs(recent))),
            }
